...
def split_data(input_folder, output_folder):
    ...
    val_end = int(len(files) * (TRAIN_RATIO + VALIDATION_RATIO))
    ...
    for set_name, files in sets.items():
        output_set_folder = os.path.join(output_folder, set_name)
        os.makedirs(output_set_folder, exist_ok=True)
        for file_path in files:
            username = os.path.basename(os.path.dirname(file_path))
            process_image(file_path, output_set_folder, username)


if __name__ == "__main__":
    split_data(base_input_folder, base_output_folder)
